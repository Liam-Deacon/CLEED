 ...
 *********************************************************************/
{
 ...
 - in the function leed_inp_overlayer_sym the symmetry is tested,so
   if the test is negative the program break
    ...
    {
      ERROR_MSG("n_rot is not set to a real positve number\n");
      ERROR_MSG("check the inputfile.\n exit now\n");
      ERROR_RETURN(-1);
    }
  ...
  Set the symmetry flags
   ...
   - The distance between the last bulk layer and the first overlayer is:
     bulk_par->layers[bulk_par->n_layers-1].vec_to_next[3] (bulk - origin)
  ...
  /************************************************************************
 Adjust structure elements dmin, vr, and n_types in bulk_par.
   *************************************************************************/
  ...
  fprintf(STDCTR,"\t%d different sets of phase shifts used:\n",
      over_par->n_types);
  for(i_c = 0; i_c < over_par->n_types; i_c ++)
