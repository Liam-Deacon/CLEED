...
**********************************************************************/
 ...
 if (matcheck(*p_Tpp)  < 0) {
#ifdef ERROR
fprintf(STDERR," *** error (leed_ms_compl): invalid input matrix (1st argument)\n");
#endif
  iaux = 1; }
 if (matcheck(*p_Tmm)  < 0) {
#ifdef ERROR
fprintf(STDERR," *** error (leed_ms_compl): invalid input matrix (2nd argument)\n");
#endif
  iaux = 1; }
 if (matcheck(*p_Rmp)  < 0) {
#ifdef ERROR
fprintf(STDERR," *** error (leed_ms_compl): invalid input matrix (3rd argument)\n");
#endif
  iaux = 1; }
 if (matcheck(*p_Rpm)  < 0) {
#ifdef ERROR
fprintf(STDERR," *** error (leed_ms_compl): invalid input matrix (4th argument)\n");
#endif
  iaux = 1; }
...
#ifdef CONTROL
 fprintf(STDCTR,"(leed_ms_compl):Hallo z_min = %f  z_max =%f lmax = %d\n",
         z_min,z_max,l_max);
#endif
...
#ifdef CONTROL
 fprintf(STDCTR,"(leed_ms_compl): l_max = %d, No of beams = %d, No of atoms = %d\n", 
...
#ifdef CONTROL
   fprintf(STDCTR,
   "(leed_ms_compl): giant matrix inversion (%d x %d), E = %.1f eV ...\n",
