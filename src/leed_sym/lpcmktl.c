...
mat *leed_par_mktl(mat *p_tl, const leed_phase *phs_shifts, size_t l_max, real energy)
...
#ifdef CONTROL
  fprintf(STDCTR,"(leed_par_mktl): energy = %.2f H, n_set = %d, l_max = %lu\n",
...
#ifdef CONTROL_X
    fprintf(STDCTR,"(leed_par_mktl):i_set = %d, lmax(set) = %d, n_eng = %ld\n",
