...
#ifdef CONTROL
 fprintf(STDCTR,
  "(leed_inp_overlayer): entering leed_inp_overlayer MIN_DIST= %.3f, n_atoms = %d\n", 
...
#ifdef CONTROL
 fprintf(STDCTR,"(leed_inp_overlayer): overlayer atoms split up into %d layers\n", 
         i_layer);
#endif
...
#ifdef ERROR
   fprintf(STDERR,
   "*** error (leed_inp_overlayer): unable to allocate memory for overlayer\n");
#endif
#ifdef EXIT_ON_ERROR
   exit(1);
#else
...
#ifdef CONTROL_X
   fprintf(STDCTR,"(leed_inp_overlayer): no_of_atoms[%d] = %d\n", i, no_of_atoms[i]);
   fprintf(STDCTR,"(leed_inp_overlayer): par->layers[%d].natoms = %d\n", 
...
#ifdef ERROR
     fprintf(STDERR,
"*** error (leed_inp_overlayer): the numbers of atoms in layer %d do not match\n", i);
#endif
#ifdef EXIT_ON_ERROR
       exit(1);
#else
...
#ifdef CONTROL
 fprintf(STDCTR,
  "(leed_inp_overlayer): leaving leed_inp_overlayer, return value = %d\n", i_layer);
#endif
