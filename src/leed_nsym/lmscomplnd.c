                     ...
                     leed_var *v_par, leed_layer *layer, leed_beam *beams)
{
  ...
  {
    #ifdef ERROR
    fprintf(STDERR, "*** error (leed_ms_compl_nd): "
  ...
  #ifdef CONTROL
  fprintf(STDCTR, "(leed_ms_compl_nd): l_max = %d, No of beams = %d, "
  ...
  for(i_atoms = 0; i_atoms < n_atoms; i_atoms ++)
  {
    ...
    {
      #ifdef CONTROL
      fprintf(STDCTR, "(leed_ms_compl_nd):  before leed_ms_tmat_ii (%d): "
              "i_type = %d", i_atoms, i_type);
      if(t_type == T_DIAG)
      {
      ...
      else
      {
        #ifdef ERROR
        fprintf(STDERR, "*** error (leed_ms_compl_nd): "
                "unidentified t_type for atom No %d\n", i_atoms);
        #endif
      ...
      i_atoms ++, off_row += l_max_2)
  {
    for(j_atoms = i_atoms + 1, off_col = off_row + l_max_2;
      ...
      if( ! IS_EQUAL_REAL(RMATEL(i_atoms+1, j_atoms+1, Mark), 1.))
      {
        ...
        #ifdef CONTROL
        fprintf(STDCTR, "(leed_ms_compl_nd): "
                "d(%d->%d) = (%5.2f, %5.2f, %5.2f) A\n",
  ...
  #ifdef CONTROL
  fprintf(STDCTR, "(leed_ms_compl_nd): giant matrix inversion (%d x %d), "
