...
mat * p_Tii;                    /* Array of Bravais layer scattering matrices */
 ...
 CTIME("(leed_ms_compl_nd): start of function\t\t");
...
**********************************************************************/
 ...
 if (matcheck(*p_Tpp)  < 0) {
#ifdef ERROR
fprintf(STDERR," *** error (leed_ms_compl_nd): invalid input matrix (1st argument)\n");
#endif
  iaux = 1; }
 if (matcheck(*p_Tmm)  < 0) {
#ifdef ERROR
fprintf(STDERR," *** error (leed_ms_compl_nd): invalid input matrix (2nd argument)\n");
#endif
  iaux = 1; }
 if (matcheck(*p_Rmp)  < 0) {
#ifdef ERROR
fprintf(STDERR," *** error (leed_ms_compl_nd): invalid input matrix (3rd argument)\n");
#endif
  iaux = 1; }
 if (matcheck(*p_Rpm)  < 0) {
#ifdef ERROR
fprintf(STDERR," *** error (leed_ms_compl_nd): invalid input matrix (4th argument)\n");
#endif
  iaux = 1; }
...
#ifdef CONTROL
 fprintf(STDCTR,"(leed_ms_compl_nd): z_min = z_max =%f n_atoms = %d\n",
         z_min,n_atoms);
#endif
...
#ifdef CONTROL
 fprintf(STDCTR,"(leed_ms_compl_nd): z_min = %f  z_max =%f lmax = %d\n",
         z_min,z_max,l_max);
#endif
...
#ifdef CONTROL
 fprintf(STDCTR,"(leed_ms_compl_nd): l_max = %d, No of beams = %d, No of atoms = %d\n", 
         l_max, n_beams, n_atoms);
 fprintf(STDCTR,"(leed_ms_compl_nd): before sorting:\n");
#endif
...
#ifdef CONTROL
   fprintf(STDCTR,"(leed_ms_compl_nd): z_plane: %.4f, n_plane: %d\n", 
...
#ifdef CONTROL
 fprintf(STDCTR,"(leed_ms_compl_nd): after sorting:\n");
 for(i_atoms = 0; i_atoms < n_atoms; i_atoms ++)
 {
...
**********************************************************************/
 ...
 {
#ifdef ERROR
   fprintf(STDERR,"*** error (leed_ms_compl_nd): Allocation error for p_Tii\n");
#endif
#ifdef EXIT_ON_ERROR
   exit(1);
#else
...
#ifdef CONTROL_X
   fprintf(STDCTR,"(leed_ms_compl_nd):  Calculate Bravais lattice sum\n");
#endif
...
#ifdef CONTROL_X
   fprintf(STDCTR,"(leed_ms_compl_nd):  Calculate scattering matrices\n");
#endif
...
*/
   ...
   {
#ifdef CONTROL
     fprintf(STDCTR,"(leed_ms_compl_nd):  before leed_ms_tmat_ii (%d): i_type = %d", 
             i_atoms, i_type);
     if(t_type == T_DIAG)
     ...
     {
       p_Tii[i_type] = 
         leed_ms_tmat_nd_ii( p_Tii[i_type], Llm_ij, v_par->p_tl[i_type], l_max);
     }
     else
     {
#ifdef ERROR
       fprintf(STDERR, 
         "*** error (leed_ms_compl_nd): unidentified t_type for atom No %d\n", 
         i_atoms);
#endif
#ifdef EXIT_ON_ERROR
       exit(1);
...
#ifdef CONTROL
   fprintf(STDCTR,"(leed_ms_compl_nd): d(%d->%d) = (%5.2f, %5.2f, %5.2f) A\n",
...
#ifdef CONTROL_LSUM
     fprintf(STDCTR,"(leed_ms_compl_nd): Lij\n");
...
*/
         ;
#ifdef CONTROL
       fprintf(STDCTR,"(leed_ms_compl_nd):         = d(%d->%d)\n", k, l);
#endif
       } /* if R_fabs ... */
     } /* for k,l */
...
/* Add identity to Mbg */
...
#ifdef CONTROL
   fprintf(STDCTR,
   "(leed_ms_compl_nd): giant matrix inversion (%d x %d), E = %.1f eV ...\n",
 ...
 CTIME("(leed_ms_compl_nd): before giant matrix inversion");
...
#ifdef CONTROL
   fprintf(STDCTR,"(leed_ms_compl_nd): ... completed\n");
#endif
 CTIME("(leed_ms_compl_nd): after giant matrix inversion");
...
#ifdef CONTROL
   fprintf(STDCTR,"(leed_ms_compl_nd): relative u.c. area: %.3f\n", layer->rel_area);
#endif
...
#ifdef CONTROL
   fprintf(STDCTR,"(leed_ms_compl_nd): Prepare matrices R_x and L_x (%d x %d)\n",
           n_beams, iaux);
#endif
...
*/
 ...
 if(i_atoms == 0)
 {
 fprintf(STDCTR,"(leed_ms_compl_nd):R_p (atom%d)\n",i_atoms);
 matshow(Maux);
 }
#endif
...
*/
 ...
 CTIME("(leed_ms_compl_nd): after preparation of R_p ... ");
 matfree(Ylm);
...
**********************************************************************/
 ...
 CTIME("(leed_ms_compl_nd): after multiplication R * Mbg * L");
...
#ifdef CONTROL
   fprintf(STDCTR,"(leed_ms_compl_nd): origin shift ... \n");
#endif
...
/* exp[-ikz(+) * (zn - z1)] */
   ...
   fprintf(STDCTR,"(leed_ms_compl_nd): (%2d) propagator: (%.3f, %.3f) [%.3f]\n", 
 ...
 *p_Rmp = Rmp;
 ...
 CTIME("(leed_ms_compl_nd): end of function");
