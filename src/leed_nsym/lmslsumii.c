...
**************************************************************************/
 ...
 {
#ifdef ERROR
   fprintf(STDERR," *** error (leed_ms_lsum_ii): damping too small: k_i = %.2e\n",
           k_i);
#endif
...
*/
 ...
 if( r_max > WARN_LEVEL)    /* poor convergence */
   fprintf(STDWAR,
     "* warning (leed_ms_lsum_ii): damping very weak: k_i = %.2e, eps = %.2e\n",
     k_i, epsilon);
#endif
...
#ifdef CONTROL
 fprintf(STDCTR,
   "(leed_ms_lsum_ii):  a1  = (%.3f,%.3f) A, a2  =  (%.3f,%.3f) A\n", 
