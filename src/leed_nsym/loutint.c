/*********************************************************************
  ...
  leed_output_int(mat Amp, leed_beam_t *beams, leed_var_t *par, FILE * outfile)
...
int leed_output_int(mat Amp, leed_beam_t *beams_now, leed_beam_t *beams_all,
...
#ifdef CONTROL
 fprintf(STDCTR,"(leed_output_int):\t     beam\t  intensity\n\t\t\t== %.2f eV ==\n", 
                par->eng_v*HART);
...
*/
 ...
 for (i_beams_all = 0; ! IS_EQUAL_REAL((beams_all + i_beams_all)->k_par, F_END_OF_LIST); 
      i_beams_all ++)
 {
