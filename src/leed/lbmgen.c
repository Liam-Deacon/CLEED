...
/*! \file
 *
 * Contains functions for setting up lists of beams used within the energy loop.
 */
...
/*!
 * Helper function for allocating memory to #leed_beam data members
 *
 * \param beam Beam to malloc data members
 * \param bulk_layers Number of bulk layers to allocate
 * \param over_layers Number of surface layers to allocate
 *
 * \warning Exits on allocation failure if #CLEED_LEAN_AND_MEAN is not defined.
 *
 */
__attribute__((nonnull)) static inline void leed_beam_data_alloc(
    const leed_beam *beam, const size_t bulk_layers,
    const size_t over_layers, const size_t l_max)
{
  const size_t two_lmax_plus_1 = 2*l_max+1;
  const size_t bulk_size = two_lmax_plus_1 * bulk_layers;
  const size_t over_size = two_lmax_plus_1 * over_layers;

  CLEED_ALLOC_CHECK(beam->eout_b_r = (real*)malloc(bulk_layers*sizeof(real)));
  CLEED_ALLOC_CHECK(beam->eout_b_i = (real*)malloc(bulk_layers*sizeof(real)));
  CLEED_ALLOC_CHECK(beam->eout_s_r = (real*)malloc(over_layers*sizeof(real)));
  CLEED_ALLOC_CHECK(beam->eout_s_i = (real*)malloc(over_layers*sizeof(real)));

  CLEED_ALLOC_CHECK(beam->ein_b_r = (real*)malloc(bulk_size*sizeof(real)));
  CLEED_ALLOC_CHECK(beam->ein_b_i = (real*)malloc(bulk_size*sizeof(real)));
  CLEED_ALLOC_CHECK(beam->ein_s_r = (real*)malloc(over_size*sizeof(real)));
  CLEED_ALLOC_CHECK(beam->ein_s_i = (real*)malloc(over_size*sizeof(real)));
}

/*!
 * Sorts the \p beams between \p offset and \p n_beams
 *
 * First sort by parallel k component, then by Miller indices (h then k).
 *
 * \param beams Pointer to array of beams.
 * \param n_beams The maximum number of beams to sort to.
 * \param offset The starting index for the sort.
 */
__attribute__((nonnull)) static void leed_beams_sort(
    leed_beam *beams, const size_t n_beams, const size_t offset)
{
  size_t n1, n2;
  leed_beam beam_aux;
   ...
   */
  for(n1 = offset; n1 < n_beams; n1 ++)
  {
    for(n2 = n1+1; n2 < n_beams; n2 ++)
    {
...
/*!
 * Wrapper function for non-symmetrized version of leed_beam_gen()
 */
inline int leed_beam_gen_nd(leed_beam **p_beams,
    const leed_crystal * const b_par, leed_var *v_par, real eng_max)
{
  return leed_beam_gen(p_beams, b_par, NULL, v_par, eng_max, false);
}

/*!
 * Wrapper function for symmetrized version of leed_beam_gen()
 */
inline int leed_beam_gen_sym(leed_beam **p_beams,
    const leed_crystal * const b_par, const leed_crystal *o_par,
    leed_var *v_par, real eng_max)
{
  return leed_beam_gen(p_beams, b_par, o_par, v_par, eng_max, true);
}
 ...
 * (re)allocated.
 */
int leed_beam_gen(leed_beam **p_beams,
    const leed_crystal * const b_par, const leed_crystal * const o_par,
    leed_var *v_par, real eng_max, bool sym)
{
  size_t n1, n2, offset;

  size_t iaux, i_rot, i_beams, i_set = 0;
  const size_t n_set = (size_t) fabs(cleed_real_nint(b_par->rel_area_sup));

  const real k_max_2 = pow(cleed_real_log(v_par->epsilon) / b_par->dmin, 2)
                          + 2*(eng_max-v_par->vr); /* note the -vr */
  const real k_max = cleed_real_sqrt(k_max_2);

  real faux_i;
  real faux_r = cleed_real_sin(v_par->theta) * cleed_real_sqrt(2*eng_max);

  /* incident momentum vector at maximum energy, k_in */
  const real k_in[3] = {faux_r, 
      faux_r * cleed_real_cos(v_par->phi),
      faux_r * cleed_real_sin(v_par->phi)};
  real k_r, k_i;
  real k_x, k_y;

  /* lattice vectors - initally set to (1x1) */
  real a1_x = b_par->a[1], a1_y = b_par->a[3];
  real a2_x = b_par->a[2], a2_y = b_par->a[4];

  real det = 1./(a1_x*a2_y - a1_y*a2_x);

  /* reciprocal (1x1) lattice vectors (g1/2_x/y) */
  const real g1_x = b_par->a_1[1], g1_y = b_par->a_1[2];
  const real g2_x = b_par->a_1[3], g2_y = b_par->a_1[4];

  const real a1 = cleed_real_hypot(g1_x, g1_y); /* a1 = length of g1 */
  const real a2 = cleed_real_hypot(g2_x, g2_y); /* a2 = length of g2 */

  /* indicies of the basic superstructure vectors (mij) */
  const real m11 = b_par->m_recip[1], m12 = b_par->m_recip[2];
  const real m21 = b_par->m_recip[3], m22 = b_par->m_recip[4];

  leed_beam *beams = NULL, beam_aux;
  leed_beam *bm_off = (leed_beam *)calloc(n_set, sizeof(leed_beam));

  /*
   * n2_max = k_max / (sin(a1,a2) * a2) + k_in/a2
   * n1_max = k_max / a1 + n2_max * (cos(a1,a2) *a2)/a1 + k_in/a1
   */
  const int n2_max = 2 + (int)
          (k_max/cleed_real_fabs((g1_x*g2_y - g1_y*g2_x)/a1) + k_in[0]/a2);
  const int n1_max = 2 + (int)(k_max/a1 +
      n2_max*cleed_real_fabs((g1_x*g2_x + g1_y*g2_y)/a1)/a1 + k_in[0]/a1);
   ...
   */
  eng_max -= v_par->vr; /* vacuum energy of the optical potential */
  iaux =  2 + (size_t)fabs( 0.10132118 * b_par->rel_area_sup *
      b_par->area * k_max_2 );

  if (*p_beams == NULL)
  {
    CLEED_ALLOC_CHECK(*p_beams = (leed_beam *)calloc(iaux, sizeof(leed_beam)));
  }
  else
  {
    CLEED_REALLOC(*p_beams, iaux*sizeof(leed_beam));
  }
  ...
  if (!sym)
    CONTROL_MSG(CONTROL_X, "set %d: %5.2f %5.2f (%5.2f %5.2f)\n", 0,
        bm_off[0].ind_1, bm_off[0].ind_2, bm_off[0].k_r[1], bm_off[0].k_r[2]);
  else
    CONTROL_MSG(CONTROL_X, "set %d: index_a (%5.2f %5.2f), "
        "index_b (%2d %2d), real (%5.2f %5.2f)\n",
        0, bm_off[0].ind_1,   bm_off[0].ind_2,
    ...
    for(n2 = -(int)n_set; (n2 <= (int)n_set) && (i_set < n_set); n2++)
    {
          ...
          (cleed_real_hypot(k_x, k_y) > K_TOLERANCE) )
      {
        ...
        if (sym)
        {
          ...
          CONTROL_MSG(CONTROL_X, "set %d: index_a (%5.2f %5.2f), ",
              "index_b (%2d %2d), real (%5.2f %5.2f)\n", i_set,
              bm_off[i_set].ind_1,   bm_off[i_set].ind_2,
  ...
  if( i_set != n_set)
  {
	...
	if (sym)
	{
	  leed_beam_gen_vars vars = {
	      .k_max = k_max, .k_max_2 = k_max_2, .k_in = k_in,
	      .a1 = a1, .a2 = a2, .eng_max = eng_max,
	      .bm_off = bm_off, .n_set = &n_set};
    ...
    switch(b_par->symmetry)
    {
       ...
       */
      case HEX_3ROT: case REC_2ROT: case MONO_2ROT:
      case SQ_2ROT: case SQ_4ROT: case NOSYM:
      {
        return leed_beam_gen_rot(beams, b_par, o_par, v_par, &vars);
        break;
      }  /* case rotational symmetry */

      /* CASE MIRRORSYMMETRY */
      case HEX_3MIR: case HEX_1MIR:
      case REC_1MIR: case REC_2MIR: case MONO_1MIR:
      case SQ_1MIR: case SQ_2MIR: case SQ_4MIR:
      {
        return leed_beam_gen_mir(beams, b_par, o_par, v_par, &vars);
        break;
      ...
      i_set ++, offset = i_beams)
  {
    /*
     * Find the beams within the radius defined by k_max
     * and within the wedge defined by symmetry (if required)
     * (loop over beam indices)
     */
    ...
    leed_beams_sort(beams, i_beams, offset);
 ...
 * needed in the routines bravelsym() and complsym()
 */
__attribute__((nonnull)) static size_t leed_beam_gen_rot(leed_beam *beams,
    const leed_crystal * const b_par,
    const leed_crystal * const o_par,
    const leed_var * const v_par,
    leed_beam_gen_vars *vars)
{
  int n1, n2, n1_max, n2_max, m;
  size_t i, j, i_layers, i_beams, i_set, j_set, i_rot, iaux, offset;

  const size_t n_mir = b_par->n_mir; /* degrees of mirror symmetry */
  const size_t n_rot = b_par->n_rot; /* degrees of rotational symmetry */
  const size_t l_max = v_par->l_max;

  const real k_max = vars->k_max, k_max_2 = vars->k_max_2, *k_in = vars->k_in;
  size_t n_set = *vars->n_set;

  real **R_n = leed_beam_get_rotation_matrices(n_rot);
  ...
  leed_beam bm_off = vars->beam_offsets;
  leed_beam beam_aux;
  ...
  const real a1 = vars->a1; /* a1 = length of g1 */
  const real a2 = vars->a2; /* a2 = length of g2 */

  /* indicies of the basic superstructure vectors (mij) */
  real m11 = b_par->m_recip[1], m12 = b_par->m_recip[2];
  real m21 = b_par->m_recip[3], m22 = b_par->m_recip[4];
  ...
  for(size_t i_set = 0; i_set < n_set; i_set ++)
  {
   ...
   */
  bm_off[0].n_eqb_s *= bm_off[0].n_eqb_b;
  ...
  *n_set = j_set;
   ...
   * n1_max = k_max / a1 + n2_max * (cos(a1,a2) *a2)/a1 + k_in/a1
   */
  int n2_max = 2 + (int)(k_max/faux_i + k_in[0]/a2);
  int n1_max = 2 + (int)(k_max/a1 + n2_max*faux_r/a1 + k_in[0]/a1);
      ...
      for(n2 = -n2_max; n2 <= n2_max; n2 ++)
      {
        ...
        if(faux_r <= k_max_2)
        {
            ...
            /* ~ 1 deg tolerance by adding +/- DEG_TO_RAD */
          {
            ...
            {
              for(size_t i_rot = 1; i_rot < n_rot; i_rot ++)
              {
             ...
             *
             * The information relevant for i_layers and m will be stored in
             *   ein_b/s[nlayers*i_layers + l_max + m]
             */

            /* Akz_r = (area of the unit cell)^-1 */
            beams[i_beams].Akz_r = 1./b_par->area;
            leed_beam_data_alloc(&beams[i_beams],
                                  b_par->n_layers, o_par->n_layers, l_max);
            ...
            for(i_layers = 0, iaux = 0; i_layers < b_par->n_layers; i_layers++)
            {
              ...
              for(int m = -l_max; m <= l_max; m ++, iaux ++)
              {
                if( bm_off[i_set].n_eqb_b == 1)
                {
                  faux_r = a1_x * beams[i_beams].k_x_sym[0]
  ...
  for(i_layers=0, iaux=0; i_layers < o_par->n_layers; i_layers++)
  {
    ...
    for(m = -(int)l_max; m <= (int)l_max; m ++, iaux ++)
    {
  ...
  return (int)(*vars->n_set = n_set);
}

__attribute__((nonnull)) static int leed_beam_gen_mir(
    leed_beam *beams,
    const leed_crystal * const b_par,
    const leed_crystal * const o_par,
    const leed_var * const v_par,
    leed_beam_gen_vars *vars)
{
  int n1, n2, n1_max, n2_max, m;
  size_t i, j, i_layers, i_beams, i_set, j_set, i_rot, iaux, offset;
  size_t i_c, i_d, i_e, ctrol;

  const size_t n_mir = b_par->n_mir; /* degrees of mirror symmetry */
  const size_t n_rot = b_par->n_rot; /* degrees of rotational symmetry */
  const size_t l_max = v_par->l_max;

  const real a1 = vars->a1, a2 = vars->a2;
  const real *k_in = vars->k_in, k_max = vars->k_max, k_max2 = vars.k_max_2;
  const real eng_max = vars->eng_max;
  ...
  for(i_set=1; i_set < n_set; i_set++)
  {
    if( bm_off[i_set].n_eqb_s != 0 )
    {
      real equalset1 = bm_off[i_set].ind_1;
      real equalset2 = bm_off[i_set].ind_2;
...
#if CONTROL
  CONTROL_MSG(CONTROL, "%d sets in symmetrised list of beam sets:\n", n_set);
  for(i_set=0; i_set < n_set; i_set++)
  {
    CONTROL_MSG(CONTROL, "set %d: (%5.2f %5.2f), (%d %d)\n",
        i_set, bm_off[i_set].ind_1,  bm_off[i_set].ind_2,
